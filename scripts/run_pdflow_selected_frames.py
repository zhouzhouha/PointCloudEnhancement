"""Run PD-Flow denoising on selected UVG-CWI-DQPC frames."""
...
PDFLOW_ROOT = common.REPO_ROOT / "third_party" / "enhancement" / "pdflow"
PDFLOW_CKPT = PDFLOW_ROOT / "pretrain" / "pdflow-score-LCC.pt"


def main():
    parser = argparse.ArgumentParser(description="Run PD-Flow selected frames")
    ...
    parser.add_argument("--method-name", default="pdflow")
    ...
    if not PDFLOW_CKPT.exists():
        raise FileNotFoundError(PDFLOW_CKPT)

    work_root = args.results_root / "work" / args.method_name / args.sequence / "15fps"
    input_root = work_root / "input"
    pdflow_root = work_root / "pdflow_results"
    ...
    for path in [input_root, pdflow_root, out_root, metric_root]:
    ...
    for frame in args.frames:
        ...
        output_xyz = pdflow_root / f"{stem}.denoised.xyz"
        common.write_xyz(input_xyz, source_points)

        common.run(
            [
                sys.executable,
                "models/deflow/denoise.py",
                "--input",
                input_xyz,
                "--output",
                output_xyz,
