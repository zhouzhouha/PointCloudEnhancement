"""Run SnowflakeNet completion on selected UVG-CWI-DQPC frames.

This is a domain-transfer adapter around the official SnowflakeNet completion
checkpoint. UVG frames are much larger than PCN/ShapeNet completion inputs, so
the adapter deterministically samples 2048 XYZ points, normalizes them into a
unit object frame, runs the pretrained completion model, denormalizes the final
output, and transfers RGB from the original CG frame with nearest neighbor
(`k=1`).
"""
...
DEFAULT_CONFIG = SNOWFLAKENET_ROOT / "completion" / "configs" / "pcn_cd1.yaml"
DEFAULT_CKPT = SNOWFLAKENET_ROOT / "pretrained" / "completion" / "ckpt-best-pcn-cd_l1.pth"


def load_snowflakenet(device: str, config_path: Path, checkpoint_path: Path):
    completion_root = SNOWFLAKENET_ROOT / "completion"
    sys.path.insert(0, str(SNOWFLAKENET_ROOT))
    sys.path.insert(0, str(completion_root))
    old_cwd = Path.cwd()
    try:
        os.chdir(completion_root)
        from core import builder  # noqa: WPS433
        from utils.yaml_reader import read_yaml  # noqa: WPS433
...
def complete_frame(model, points: np.ndarray, device: str, n_input: int, seed: int):
    ...
    with torch.no_grad():
        ...
        completed = outputs[-1].squeeze(0).detach().cpu().numpy().astype(np.float32)
    completed = completed * scale + centroid
    return completed.astype(np.float32), sampled


def main():
    parser = argparse.ArgumentParser(description="Run SnowflakeNet selected frames")
    ...
    parser.add_argument("--method-name", default="snowflakenet_pcn_cd1")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("SnowflakeNet smoke requires CUDA.")

    device = "cuda"
    torch.manual_seed(args.seed)
    np.random.seed(args.seed)
    model = load_snowflakenet(device, args.model_config, args.checkpoint)
