"""Run SeedFormer completion on selected UVG-CWI-DQPC frames.

This adapter keeps the official pretrained SeedFormer model intact and only
handles UVG-specific I/O: deterministic 2048-point sampling, unit-object
normalization, denormalization, k=1 RGB transfer from the CG frame, and metric
bookkeeping against the HE reference.
"""
...
SEEDFORMER_ROOT = common.REPO_ROOT / "third_party" / "enhancement" / "SeedFormer"
SEEDFORMER_CODES = SEEDFORMER_ROOT / "codes"
DEFAULT_CKPT = SEEDFORMER_ROOT / "pretrained" / "pcn" / "ckpt-best.pth"


def load_seedformer(device: str, checkpoint_path: Path):
    sys.path.insert(0, str(SEEDFORMER_CODES))
    old_cwd = Path.cwd()
    try:
        os.chdir(SEEDFORMER_CODES)
        import model as seedformer_model  # noqa: WPS433

        model = seedformer_model.seedformer_dim128(up_factors=[1, 4, 8])
...
def complete_frame(model, points: np.ndarray, device: str, n_input: int, seed: int):
    ...
    with torch.no_grad():
        completed = model(tensor)[-1].squeeze(0).detach().cpu().numpy().astype(np.float32)
    completed = completed * scale + centroid
    return completed.astype(np.float32), sampled


def main():
    parser = argparse.ArgumentParser(description="Run SeedFormer selected frames")
    ...
    parser.add_argument("--method-name", default="seedformer_pcn")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("SeedFormer smoke requires CUDA.")

    device = "cuda"
    torch.manual_seed(args.seed)
    np.random.seed(args.seed)
    model = load_seedformer(device, args.checkpoint)
