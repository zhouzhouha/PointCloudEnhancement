"""Run DMRDenoise on selected UVG-CWI-DQPC frames.

This is an adapter around the released DMRDenoise `denoise.py`. It prepares UVG
CG frames as `.xyz`, calls the official denoising entrypoint with the released
supervised checkpoint and default denoising settings, then writes XYZRGB PLY
files by copying the nearest original CG color (`k=1`) to every denoised point.
"""
...
DMR_ROOT = REPO_ROOT / "third_party" / "enhancement" / "DMRDenoise"
DMR_CKPT = DMR_ROOT / "pretrained" / "supervised" / "epoch=153.ckpt"
...
def patch_dmr_sklearn_compat(dmr_module):
    """Remove removed sklearn KMeans kwargs while preserving DMRDenoise logic."""
    sklearn_kmeans = dmr_module.KMeans

    def compat_kmeans(*args, n_jobs=None, **kwargs):
        return sklearn_kmeans(*args, **kwargs)

    dmr_module.KMeans = compat_kmeans


def run_dmrdenoise(input_xyz: Path, output_xyz: Path, args):
    sys.path.insert(0, str(DMR_ROOT))
    import denoise as dmr_denoise  # noqa: PLC0415

    patch_dmr_sklearn_compat(dmr_denoise)
    dmr_args = SimpleNamespace(
        input=str(input_xyz),
        output=str(output_xyz),
        ckpt=str(args.ckpt),
    ...
    dmr_denoise.auto_denoise(dmr_args)


def main():
    parser = argparse.ArgumentParser(description="Run DMRDenoise selected frames")
    ...
    parser.add_argument("--method-name", default="dmrdenoise")
    parser.add_argument("--ckpt", type=Path, default=DMR_CKPT)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--expand-knn", type=int, default=16)
    ...
    if not args.ckpt.exists():
        raise FileNotFoundError(f"Missing DMRDenoise checkpoint: {args.ckpt}")

    work_root = REPO_ROOT / "results" / "work" / args.method_name / args.sequence / "15fps"
    input_root = work_root / "input"
    dmr_root = work_root / "dmr_results"
    ...
    for path in [input_root, dmr_root, out_root, metric_root]:
    ...
    for frame in args.frames:
        ...
        input_xyz = input_root / f"{shape_base}.xyz"
        output_xyz = dmr_root / f"{shape_base}.denoised.xyz"
        write_xyz(input_xyz, source_points)

        print(f"[dmrdenoise] frame={frame} input={input_xyz} output={output_xyz}", flush=True)
        run_dmrdenoise(input_xyz, output_xyz, args)
        ...
        dmr = eval_pointcloud(str(out), str(he), samplepoint=0, eval_type="ply", thresholds=[5, 10, 20])
        rows.append({"method": "cg_baseline", "sequence": args.sequence, "frame": frame, "pred_file": str(cg), "gt_file": str(he), **baseline})
        rows.append({"method": args.method_name, "sequence": args.sequence, "frame": frame, "pred_file": str(out), "gt_file": str(he), **dmr})
        for metric in baseline:
            delta, improved = compare_metric(metric, baseline[metric], dmr[metric])
            comparisons.append(
                {
                    "frame": frame,
                    "metric": metric,
                    "baseline": baseline[metric],
                    args.method_name: dmr[metric],
