...
# Set CPU mode before importing torch if CUDA_VISIBLE_DEVICES is empty
if os.environ.get('CUDA_VISIBLE_DEVICES') == '':
    import torch
    # Ensure torch doesn't try to use CUDA
else:
    import torch
...
class BaseTrainRunner():
    def __init__(self,**kwargs):
        ...
        # Force CPU mode if GPU_INDEX is -1 or if CUDA is not available
        if str(self.GPU_INDEX) == '-1' or not torch.cuda.is_available():
            os.environ["CUDA_VISIBLE_DEVICES"] = ''
            self.GPU_INDEX = '-1'
        elif (not self.GPU_INDEX == 'all'):
