...
def plot_surface(with_points,points,decoder,latent,path,epoch,in_epoch,shapefile,resolution,mc_value,is_uniform_grid,verbose,save_html,save_ply,overwrite):
    ...
    if (not os.path.exists(filename) or overwrite):
        ...
        surface = get_surface_trace(points,decoder,latent,resolution,mc_value,is_uniform_grid,verbose,save_ply)
        trace_surface = surface["mesh_trace"]
        layout = go.Layout(title= go.layout.Title(text=shapefile), width=1200, height=1200, scene=dict(xaxis=dict(range=[-2, 2], autorange=False),
                                                               yaxis=dict(range=[-2, 2], autorange=False),
                                                               zaxis=dict(range=[-2, 2], autorange=False),
                                                               aspectratio=dict(x=1, y=1, z=1)))
        if (with_points):
...
def get_surface_trace(points,decoder,latent,resolution,mc_value,is_uniform,verbose,save_ply):
    ...
    if (is_uniform):
        grid = get_grid_uniform(None,resolution)
    else:
...
def plot_cuts_axis(points,decoder,latent,path,epoch,near_zero,axis,file_name_sep='/'):
    ...
    position_cut = [position_cut + i*mask.reshape(-1, 1) for i in np.linspace(min_axis - 0.1, max_axis + 0.1, 3)]
...
def plot_cuts(points,decoder,path,epoch,in_epoch,near_zero,latent):
    ...
    position_cut = np.vstack(([xx, np.zeros(xx.shape[0]), yy]))
    position_cut = [position_cut + np.array([0., i, 0.]).reshape(-1, 1) for i in np.linspace(min_y - 0.1, max_y + 0.1, 2)]
    for index, pos in enumerate(position_cut):
        #fig = tools.make_subplots(rows=1, cols=1)
